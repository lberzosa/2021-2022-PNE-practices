...
class TestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/": # son los endpoints
            ...
            try:
                species = str(arguments['specie'][0].strip()) #quitamos espacios para que lo lea, solo lee el espacio sino
            ...
            try:
                ...
                for i in range(0, len(answer)):
                    for c in answer[i]["phenotype_associations"]:  # para las keys in la lista de phenotype
                        if "attributes" in c:  # si existe esa key
                            if "associated_gene" in c["attributes"]:  # si existe ese value dentro de esa key
                                final_gene.append(c["attributes"]["associated_gene"])  # appendear los values y las keys
                if "json" in arguments:
        ...
        if "json" in arguments.keys(): #MIRAR ESTO
